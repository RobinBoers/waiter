...
const _AUTH_USERNAME: &str = "root";
const _AUTH_PASSWORD: &str = "toor";
...
async fn handle_request(request: Req) -> Result<Resp, Infallible> {
    match *request.method() {
        ...
        _ => Ok(serve(400, "Bad request; only `GET` and `PUT` requests are supported."))
    }
}

async fn handle_put_request(_request: Req) -> Result<Resp, Infallible> {
   todo!() 
}

async fn handle_get_request(request: Req) -> Result<Resp, Infallible> {
    let mut response = try_files(&request).await;
    let url = request.uri();
...
async fn try_files(request: &Req) -> Resp {
...
async fn try_index(path: &str) -> Option<Resp> {
    let path = find_index(path)?;
    serve_path(&path).await
}

fn find_index(base_path: &str) -> Option<String> {
    let possible_indexes = vec![
        format!("{base_path}/index.htmd"),
        format!("{base_path}/index.txt"),
        format!("{base_path}/index.html"), 
        format!("{base_path}/index.xml"),
    ];
...
async fn serve_path(path: &str) -> Option<Resp> {
    let scope = Path::new(SCOPE).canonicalize().unwrap();
...
async fn serve_file(filename: &str, mime_type: &str) -> Result<Resp, String> {
    if let Ok(contents) = tokio::fs::read(filename).await {
        ...
        let response =  Response::builder()
            .status(200)
            .header("Content-Type", format!("{mime_type}; charset={CONTENT_CHARSET}"))
            .body(Full::new(body))
            .unwrap();
...
fn serve(status_code: u16, message: &str) -> Resp {
...
fn set_cache_time(response: &mut Resp, request_url: &str) {
    ...
    headers.insert("Cache-Control", HeaderValue::from_str(&header_value).unwrap());
}
...
fn set_additional_headers(response: &mut Resp) {
    ...
    headers.insert("Content-Language", HeaderValue::from_static(CONTENT_LANGUAGE));
}
